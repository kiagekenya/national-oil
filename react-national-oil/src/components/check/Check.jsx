...
const Check = () => {
  ...
  const [confirmStaffNumber, setConfirmStaffNumber] = useState('');
  ...
  const axiosPostData = async () => {
    ...
    await axios.post('http://localhost:4000/employee', postData )
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (staffNumber === confirmStaffNumber) {
     axiosPostData();
      alert(`Hey, checkIn for staff number ${staffNumber} recorded! Good day \u{1F600}.`);
      console.log('Staff number submitted:', staffNumber);
      setStaffNumber('');
      setConfirmStaffNumber('');
    } else {
            ...
            <form className="form" id="myform"   onKeyDown={handleKeyDown}>
              ...
              <div className="input-box">
                ...
                <input
                  type="text"
                  required
                  id="confirm-staff-number-input"
                  name="confirm"
                  value={confirmStaffNumber}
                  onChange={(e) => setConfirmStaffNumber(e.target.value)}
                />
